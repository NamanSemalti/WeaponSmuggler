...
namespace itsmakingthings_daynightcycle
{
	public class DayNightCycle : MonoBehaviour
	{
		...
		private Material _skyboxBlendMaterial;
		...
		void Start()
		{
			...
			_skyboxBlendMaterial = new Material(Shader.Find("Skybox/Blend"));
			RenderSettings.skybox = _skyboxBlendMaterial;
		...
		private void UpdateLighting()
		{
			...
			if (_skyboxBlendMaterial != null && daySkybox != null && nightSkybox != null)
			{
				float blend_1 = 0f;
				float sunsetBlend = 0f;

				// Define transition phases
				if (_timeOfDay < 18f)
				{
					// Day to sunset tint (5 PM to 6 PM)
					sunsetBlend = Mathf.InverseLerp(17f, 18f, _timeOfDay);

					// Modify tint and exposure during sunset
					Color sunsetTint = Color.Lerp(Color.white, new Color(1f, 0.65f, 0.4f), sunsetBlend); // warm orange tint
					float exposure = Mathf.Lerp(1.2f, 0.8f, sunsetBlend);

					// Apply to the day skybox
					daySkybox.SetColor("_Tint", sunsetTint);
					daySkybox.SetFloat("_Exposure", exposure);

					// No night blend yet
					blend_1 = 0f;
				}
				else if (_timeOfDay < 20f)
				{
					// Blend day → night between 6 PM – 8 PM
					blend_1 = Mathf.InverseLerp(18f, 20f, _timeOfDay);
				}
				else
				{
					// Fully night
				...
				_skyboxBlendMaterial.SetTexture("_Skybox1", daySkybox.GetTexture("_Tex"));
				_skyboxBlendMaterial.SetTexture("_Skybox2", nightSkybox.GetTexture("_Tex"));
				_skyboxBlendMaterial.SetFloat("_Blend", blend_1);
				// _skyboxBlendMaterial.SetFloat("_Exposure", 1f);
